...
SELECT SUM(SUCCESS_AMOUNT) AS successful, SUM(SETTLED_AMOUNT) AS settled,
       SUM(SUCCESS_AMOUNT) - SUM(SETTLED_AMOUNT) AS gap,
       SUM(GAP_PARTIAL_AMT) AS partially_settled, SUM(GAP_PENDING_AMT) AS pending,
       SUM(GAP_FAILED_AMT) AS settlement_failed, SUM(GAP_UNSETTLED_AMT) AS never_settled
FROM GOLD.AGG_MERCHANT_DAILY;
