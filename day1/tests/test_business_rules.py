"""BUSINESS-RULE TESTS - the cases listed in the brief (Day 1 Task 4A + Day 2 Task 1).

Each test writes a few hand-made rows, runs the REAL pipeline on them, and checks
what landed in Gold or in the data-quality log.
"""
...
def test_failed_transaction_is_not_counted_in_kpis(tiny_db):
...
def test_late_event_is_kept_with_a_warning(tiny_db):
    # the brief's example: happened 10:02:15, received 10:08:41 (more than 5 minutes late)
    q = tiny_db({"transactions": [txn()],
                 "payment_events": [evt("E1", ts="2026-09-01 10:02:15", received="2026-09-01 10:08:41")]})
    assert q("SELECT is_late FROM gold.fact_payment_event") == [(True,)]
