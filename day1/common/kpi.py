...
def settlement_rate(settled_amount: float, success_amount: float) -> float:
    """Settled amount / successful amount x 100, capped to 0..100."""
    if success_amount <= 0:
        return 0.0
    return round(max(0.0, min(settled_amount / success_amount * 100, 100.0)), 2)


def settlement_gap(success_amount: float, settled_amount: float) -> float:
    return round(max(success_amount - settled_amount, 0.0), 2)
