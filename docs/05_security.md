...
|---|---|---|---|
| **SQL injection** | `f"... WHERE merchant_id = '{merchant_id}'"` lets a caller run their own SQL | All values are **bind parameters** (`%s`). `merchant_id` must also match `^M\d{3,6}$` or the API returns 422 before touching the DB. Table names in f-strings come only from constants (marked `# nosec`). | `api/repository.py`, `api/main.py` |
...
| **Excessive logging** | Logs full of card/customer data | API logs only method, path, status, duration. Errors log exception **type** only. Pipeline prints counts, never rows. | `api/main.py`, `run_pipeline.py` |
...
Automated checks: **Bandit** (SAST) and **pip-audit** (vulnerable dependencies) run in CI.
