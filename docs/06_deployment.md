...
GitLab (merge to main)
  │
  ▼
CI pipeline ── unit + API tests ── SAST ── dependency scan ── docker build & push (tag = commit SHA)
  │
  ▼
...
| **Smoke test** | After each deploy CI calls `/health` and `/api/v1/merchant-exceptions` with the API key; failure stops promotion. |
